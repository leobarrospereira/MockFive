...
public extension Mock {
    ...
    public func stub<T: ExpressibleByNilLiteral>(identifier: String, arguments: Any?..., function: String = #function, returns: ([Any?]) -> T = { _ in nil }) -> T {
        ...
        if let registeredStub = mockBlocks[mockFiveLock]?[identifier] {
            guard let typecastStub = registeredStub as? ([Any?]) -> T else { fatalError("MockFive: Incompatible block of type '\(type(of: registeredStub))' registered for function '\(identifier)' requiring block type '([Any?]) -> \(T.self)'") }
    ...
    public func stub(identifier: String, arguments: Any?..., function: String = #function, returns: ([Any?]) -> () = { _ in }) {
        ...
        if let registeredStub = mockBlocks[mockFiveLock]?[identifier] {
            guard let typecastStub = registeredStub as? ([Any?]) -> () else { fatalError("MockFive: Incompatible block of type '\(type(of: registeredStub))' registered for function '\(identifier)' requiring block type '([Any?]) -> ()'") }
...
fileprivate func stringify(_ function: String, arguments: [Any?], returnType: String?) -> String {
    ...
    if .none == function.rangeOfCharacter(from: CharacterSet(charactersIn: "()")) {
        ...
        invocation = invocation.substring(to: invocation.index(invocation.endIndex, offsetBy: -2)) + ")"
        if let returnType = returnType { invocation += " -> \(returnType)" }
