...
class MockFiveSpecs: QuickSpec {
    override func spec() {
        ...
        describe("stubbing method implementations") {
            ...
            let fatalErrorDispatch = { (behavior: () -> ()) in DispatchQueue.global(UInt32(QOS_CLASS_USER_INITIATED.rawValue), 0).asynchronously(DispatchQueue.globalexecute: behavior) }
            
            beforeEach { FatalErrorUtil.replaceFatalError({ (message, _, _) -> Never in fatalErrorString = message }) }
